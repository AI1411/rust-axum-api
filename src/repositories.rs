...
mod test {
    ...
    async fn todo_crud_scenario() {
        ...
        let id = 1;
        let expected_todo = Todo::new(id, text.clone());
        ...
        assert_eq!(expected_todo, todo);

        // find
        let todo = repository.find(id).await.expect("failed to find todo");
        assert_eq!(expected_todo, todo);

        // all
        let todos = repository.all().await.expect("failed to get all todos");
        assert_eq!(vec![expected_todo], todos);
